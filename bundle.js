  ...
  function add_entry ({ data, first, last, space = '' }) {
    ...
    async function listen({ emo, pos, type, on, ...rest }, i) {
      ...
      if(emo.classList.contains('inp')){
        gap = gap.slice(0, -36) + `<span class="hi_space${count}">&emsp;</span><span class="hi_xpace${count}">│&nbsp;</span>`+gap.slice(-30)
        style_hi.innerHTML = `.hi_xpace${count}{display: none;}`
      }
      else if(emo.classList.contains('out')){
        gap = gap.slice(0, -36) + `<span class="lo_space${count}">&emsp;</span><span class="lo_xpace${count}">│&nbsp;</span>`+gap.slice(-30)
