  ...
  1: [function (require, module, exports) {
    (function (global) {
      ...
      function (callback) { return setTimeout(callback, 1000 / 60) }
            ...
            const clearLoop = window.cancelAnimationFrame || window.mozCancelAnimationFrame || clearTimeout
	    ...
	    horizontalSpeed: null,
            ...
            if (self.options.wrapper) {
              if (!self.options.wrapper.nodeType) {
                const wrapper = document.querySelector(self.options.wrapper)
            ...
            const getCurrentBreakpoint = function (w) {
              const bp = self.options.breakpoints
              if (w < bp[0]) return 'xs'
            ...
            var createBlock = function (el) {
              ...
              const dataXsSpeed = el.getAttribute('data-rellax-xs-speed')
              const dataMobileSpeed = el.getAttribute('data-rellax-mobile-speed')
              const dataTabletSpeed = el.getAttribute('data-rellax-tablet-speed')
              const dataDesktopSpeed = el.getAttribute('data-rellax-desktop-speed')
              const dataVerticalSpeed = el.getAttribute('data-rellax-vertical-speed')
              const dataHorizontalSpeed = el.getAttribute('data-rellax-horizontal-speed')
              const dataVericalScrollAxis = el.getAttribute('data-rellax-vertical-scroll-axis')
              const dataHorizontalScrollAxis = el.getAttribute('data-rellax-horizontal-scroll-axis')
              ...
              const dataMinX = el.getAttribute('data-rellax-min-x')
              const dataMaxX = el.getAttribute('data-rellax-max-x')
              const dataMinY = el.getAttribute('data-rellax-min-y')
              const dataMaxY = el.getAttribute('data-rellax-max-y')
              let mapBreakpoints
              ...
              const posY = self.options.vertical ? (dataPercentage || self.options.center ? wrapperPosY : 0) : 0
              const posX = self.options.horizontal ? (dataPercentage || self.options.center ? self.options.wrapper ? self.options.wrapper.scrollLeft : (window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft) : 0) : 0
              ...
              let percentageY = dataPercentage || (posY - blockTop + screenY) / (blockHeight + screenY)
              let percentageX = dataPercentage || (posX - blockLeft + screenX) / (blockWidth + screenX)
              if (self.options.center) { percentageX = 0.5; percentageY = 0.5 }

              // Optional individual block speed as data attr, otherwise global speed
              const speed = (breakpoints && mapBreakpoints[currentBreakpoint] !== null) ? Number(mapBreakpoints[currentBreakpoint]) : (dataSpeed || self.options.speed)
              const verticalSpeed = dataVerticalSpeed || self.options.verticalSpeed
              const horizontalSpeed = dataHorizontalSpeed || self.options.horizontalSpeed

              // Optional individual block movement axis direction as data attr, otherwise gobal movement direction
              const verticalScrollAxis = dataVericalScrollAxis || self.options.verticalScrollAxis
              const horizontalScrollAxis = dataHorizontalScrollAxis || self.options.horizontalScrollAxis

              const bases = updatePosition(percentageX, percentageY, speed, verticalSpeed, horizontalSpeed)
            ...
            var updatePosition = function (percentageX, percentageY, speed, verticalSpeed, horizontalSpeed) {
              const result = {}
              const valueX = ((horizontalSpeed || speed) * (100 * (1 - percentageX)))
              const valueY = ((verticalSpeed || speed) * (100 * (1 - percentageY)))
            ...
            var animate = function () {
              ...
              for (let i = 0; i < self.elems.length; i++) {
                ...
                const verticalScrollAxis = blocks[i].verticalScrollAxis.toLowerCase()
                const horizontalScrollAxis = blocks[i].horizontalScrollAxis.toLowerCase()
                const verticalScrollX = verticalScrollAxis.indexOf('x') != -1 ? posY : 0
                const verticalScrollY = verticalScrollAxis.indexOf('y') != -1 ? posY : 0
                const horizontalScrollX = horizontalScrollAxis.indexOf('x') != -1 ? posX : 0
                const horizontalScrollY = horizontalScrollAxis.indexOf('y') != -1 ? posX : 0

                const percentageY = ((verticalScrollY + horizontalScrollY - blocks[i].top + screenY) / (blocks[i].height + screenY))
                const percentageX = ((verticalScrollX + horizontalScrollX - blocks[i].left + screenX) / (blocks[i].width + screenX))
                ...
                const zindex = blocks[i].zindex

                // Move that element
                // (Set the new translation and append initial inline transforms.)
                const translate = 'translate3d(' + (self.options.horizontal ? positionX : '0') + 'px,' + (self.options.vertical ? positionY : '0') + 'px,' + zindex + 'px) ' + blocks[i].transform
...
******************************************************************************/
  ...
  <style></style>`
    ...
    }).call(this, '/src/app.js')
  }, { STATE: 3, footer: 4, header: 9, io: 10, theme_widget: 14, topnav: 16 }],
 ...
 */
    ...
    function STATE (address, modulepath) {
      ...
      function statedb (fallback) {
        ...
        if (data._) { status.open_branches[modulepath] = Object.keys(data._).length }
      ...
      function append_tree_node (id, status) {
        ...
        if (name) {
          if (status.tree_pointers[super_id]) {
            ...
            let temp_name; let new_name = name
      ...
      function get_instance_data (sid) {
        ...
        let sanitized_data; let updated_status = status
        if (status.fallback_check) {
      ...
      function validate_and_preprocess ({ fallback, xtype, pre_data = {}, orphan_check, fun_status, entries }) {
        const { id: pre_id, hubs: pre_hubs, mapping } = pre_data
        let fallback_data
            ...
            if (entry._) {
              ...
              Object.entries(entry._).forEach(([local_id, value]) => {
                Object.entries(value).forEach(([key, override]) => {
                  if (key === 'mapping' || typeof (override) === 'object') { return }
            ...
            if (entry.drive) {
              ...
              Object.entries(entry.drive).forEach(([dataset_type, dataset]) => {
                ...
                entry.inputs.forEach(dataset_id => {
                  ...
                  if (ds.type === new_dataset.type) { check_name = false }
                })
          ...
          if (expected_key_names) {
            expected_key_names.split('|').forEach(expected_key_name => {
              ...
              if (value !== undefined) {
                ...
                if (expected_types.includes(type)) { type === 'object' && errors.push(...validate_shape(value, expected_value, expected_key_name, path + '/' + expected_key_name)) } else { console.error(`Type mismatch: Expected "${expected_types.join(' or ')}" got "${type}" for key "${expected_key_name}" at:`, obj, 'of', path) }
              }
            })
          } else if (required) {
            values.forEach((value, index) => {
              ...
              if (expected_types.includes(type)) { type === 'object' && errors.push(...validate_shape(value, expected_value, keys[index], path + '/' + keys[index])) } else { console.error(`Type mismatch: Expected "${expected_types.join(' or ')}" got "${type}" for key "${keys[index]}" at: `, obj, 'of', path) }
            })
          }

          if (absent && required) {
            if (expected_key_names) { errors.push(`Can't find required key "${expected_key_names.replace('|', ' or ')}" at: `, obj, 'of', path) } else { errors.push(`No subs found for super key "${super_node}" at sub:`, obj, 'of', path) }
          }
        })
        return errors
      }
        ...
        tree._ && Object.entries(tree._).forEach(([type, instances]) => {
          ...
          Object.entries(instances).forEach(([id, override]) => {
            if (typeof (override) === 'function') {
              ...
              } else { overrides[resultant_path] = { fun: [override], by: [id] } }
            } else {
...
******************************************************************************/
        ...
        async function footer (opts) {
          ...
          // OPTS
          // ----------------------------------------
          const island = await graphic('island', './src/node_modules/assets/svg/deco-island.svg')
          // ----------------------------------------
          // TEMPLATE
          // ----------------------------------------
          const el = document.createElement('div')
    ...
    }).call(this, '/src/node_modules/footer/footer.js')
  }, { './instance.json': 5, STATE: 3, graphic: 8, io: 10 }],
...
******************************************************************************/
  ...
  ******************************************/
          ...
          async function add_root (compact) {
            ...
            if (compact) { root_entries.forEach((data, i) => add_entry_compact({ hub_el: main, data, last: i === root_entries.length - 1, ancestry: [] })) } else { root_entries.forEach((data, i) => add_entry({ hub_el: main, data, last: i === root_entries.length - 1, ancestry: [] })) }
          }
  ...
  ******************************************/
    ...
    <div class="slot_list">
      ...
      <span class="name odd">${data.name}</span>
          ...
          function add_entry_compact ({ hub_el, data, first, last, space = '', pos, ancestry }) {
            ...
            const space_handle = []; const els = []
            let slot_no = 0; let slot_on
    ...
    `
            ...
            if (getComputedStyle(type_emo, '::before').content === 'none') { type_emo.innerHTML = `[${status.entry_types[data.type]}]` }
            ...
            async function handle_slot (pair, i) {
              ...
              pair.forEach((x, j) => {
                ...
                slot_handle.push(() => {
                  ...
                  if (slot_check[0] && slot_check[1]) { slot_emo.children[0].innerHTML = '┼' } else if (slot_check[0] && !slot_check[1]) { slot_emo.children[0].innerHTML = '┴' } else if (!slot_check[0] && slot_check[1]) { slot_emo.children[0].innerHTML = '┬' } else {
              ...
              if (getComputedStyle(slot_emo, '::before').content === 'none') { slot_emo.innerHTML = `<span>─</span><span>${slot_no}─</span>` }
            }
            ...
            async function handle_slot (pair, i) {
              ...
              pair.forEach((x, j) => {
                ...
                slot_handle.push(() => {
                  ...
                  // emo_on && space_handle[i]()
                  slot_check[j] = emo_on = !emo_on
                  if (slot_check[0] && slot_check[1]) { slot_emo.children[1].innerHTML = '┼' } else if (slot_check[0] && !slot_check[1]) { slot_emo.children[1].innerHTML = '┴' } else if (!slot_check[0] && slot_check[1]) { slot_emo.children[1].innerHTML = '┬' } else {
              ...
              if (getComputedStyle(slot_emo, '::before').content === 'none') { slot_emo.innerHTML = `<span>${slot_no}─</span><span>─</span>` }
            }
  ...
  ******************************************/
          function handle_focus (e) {
            ...
            state.xtask.addEventListener('blur', e => {
              if (e.relatedTarget && e.relatedTarget.classList.contains('noblur')) { return }
          ...
          function handle_click ({ el, data, pos, hub_id, type = 'entry', ...rest }) {
            ...
            if (data && el.children.length < 1) {
              ...
              data.forEach((value, i) => on_add[type]({ hub_el: el, data: { ...status.graph[value], hub_id }, first: pos ? i === 0 : false, last: pos ? false : length === i, pos, ...rest }))
            }
          }
          ...
          async function handle_add (data) {
            ...
            let node; let task_id; let space = ''; let grand_last = true; let root = true
  ...
  ******************************************/
          async function jump (e) {
            ...
            if (el) { el.focus() } else {
              const path = []
              let temp
              for (temp = status.graph[target_id]; temp.hub; temp = status.graph[temp.hub[0]]) { path.push(temp.id) }
          ...
          function recurse (id) {
            if (state.track.includes(id)) { return }
            state.result.push(graph[id])
            state.track.push(id)
            for (temp = 0; graph[id].sub && temp < graph[id].sub.length; temp++) { recurse(graph[id].sub[temp]) }
            for (temp = 0; graph[id].inputs && temp < graph[id].inputs.length; temp++) { recurse(graph[id].inputs[temp]) }
            for (temp = 0; graph[id].outputs && temp < graph[id].outputs.length; temp++) { recurse(graph[id].outputs[temp]) }
          }
          /******************************************
   Communication
  ******************************************/
    ...
    }).call(this, '/src/node_modules/graph_explorer/graph_explorer.js')
  }, { STATE: 3, helper: 7, io: 10, localdb: 12 }],
...
******************************************************************************/
        ...
        async function header (opts) {
          ...
          const sunRellax = new Rellax(sun, { speed: 2 })
          const cloud1Rellax = new Rellax(cloud1, { speed: 4 })
          const cloud2Rellax = new Rellax(cloud2, { speed: 2 })
          const cloud3Rellax = new Rellax(cloud3, { speed: 4 })
          const cloud4Rellax = new Rellax(cloud4, { speed: 2 })
          const cloud5Rellax = new Rellax(cloud5, { speed: 4 })
          const cloud6Rellax = new Rellax(cloud6, { speed: 3 })
          const cloud7Rellax = new Rellax(cloud7, { speed: 3 })
          // ----------------------------------------
          // TEMPLATE
          // ----------------------------------------
          const el = document.createElement('div')
  ...
  <style></style>`
    ...
    }).call(this, '/src/node_modules/header/header.js')
  }, { STATE: 3, graphic: 8, io: 10, rellax: 1 }],
   ...
   */
      ...
      function read_all (keys) {
        ...
        Object.entries(localStorage).forEach(([key, value]) => {
          if (key.includes(address)) { result[key.split('/').at(-1)] = JSON.parse(value) }
        })
        return result
      }
      function drop (keys) {
        if (keys.length > 1) {
          ...
          if (Array.isArray(temp)) { temp.splice(keys[keys.length - 1], 1) } else { delete (temp[keys[keys.length - 1]]) }
          localStorage[keys[0]] = JSON.stringify(data)
        } else { delete (localStorage[keys[0]]) }
      }
        ...
        const target_key = Object.keys(localStorage).find(key => {
          if (key.includes(address)) {
            ...
            Object.entries(filters).some(([search_key, value]) => {
              if (entry[search_key] !== value) { return }
              count++
            })
            if (count === Object.keys(filters).length) {
              if (index_count === index) { return key }
              index_count++
            }
          }
        }, undefined)
...
******************************************************************************/
  ...
  <main>
      ...
      <span class="plus">+</span>
          ...
          async function save_pref () {
            ...
            Array.from(tabs.children).forEach(tab => {
              if (tab.dataset.access === 'uniq') {
                ...
                } else { pref[status.instance_id].push({ theme: tab.dataset.theme, id: tab.dataset.id, local: status.themes.builtin.includes(tab.dataset.theme) }) }
              } else { pref[status.title].push({ theme: tab.dataset.theme, id: tab.dataset.id, local: status.themes.builtin.includes(tab.dataset.theme) }) }
            })
            db.add(['pref'], pref)
          }
          ...
          async function on_inject () {
            if (status.active_tab.dataset.type === 'json') {
              ...
              } else { send({ type: 'inject', to: status.node_data.hub_id, data: status.textarea.value }) }
            }
          }
          ...
          async function init ({ data }) {
            ...
            if (data.type === 'json' || !data.file) { value = JSON.stringify(value, null, 2) }
          ...
          async function add_tab (id, value = '', access = 'uniq', theme = 'default') {
            ...
            tab.tabIndex = '0'
            tab.onkeydown = e => {
              if (e.key === 'ArrowRight' && tab.nextElementSibling) { tab.nextElementSibling.after(tab) } else if (e.key === 'ArrowLeft' && tab.previousElementSibling) { tab.previousElementSibling.before(tab) }
              tab.focus()
            }
          ...
          async function close_tab (tab) {
            ...
            tab.remove()
            if (tabs.children.length) { switch_tab(tabs.children[tabs.children.length - 1].id) } else { add_tab('New') }
          }
          ...
          async function rename (e) {
            ...
            input.onblur = e => {
              if (e.relatedTarget) { btn.innerHTML = hub.dataset.name }
            }
            input.focus()
          }
    ...
    }).call(this, '/src/node_modules/theme_editor/theme_editor.js')
  }, { STATE: 3, io: 10, localdb: 12 }],
...
******************************************************************************/
  ...
  <style></style>`
          ...
          async function click ({ data }) {
            ...
            if (status.instance_id === data.id) { editor.classList.toggle('active') } else {
    ...
    }).call(this, '/src/node_modules/theme_widget/theme_widget.js')
  }, { STATE: 3, graph_explorer: 6, io: 10, theme_editor: 13 }],
...
******************************************************************************/
    ...
    }).call(this, '/src/node_modules/topnav/topnav.js')
  }, { './instance.json': 15, STATE: 3, graphic: 8, io: 10 }],
...
******************************************************************************/
    ...
    }).call(this, '/web/demo.js', '/web')
  }, { '../src/app': 2, '../src/node_modules/STATE': 3 }]
}, {}, [17])
